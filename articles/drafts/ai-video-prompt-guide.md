...
> 💬 **現場の本音**：動画生成で最も差がつくのは「カメラワークの指定」です。同じ被写体でも、`static shot`と`dolly in`では印象がまったく違います。映画を見るとき「カメラがどう動いているか」を意識すると、プロンプト力が一気に上がります。

画像生成のプロンプトと比較したい方は[AI画像生成プロンプト完全ガイド](/ai-image-prompt-guide/)も参考にしてください。
...
> 💡 **プロの使い方**：AI動画は「完成品」ではなく「素材」として使うのが正解です。5〜10秒のクリップを複数生成し、Premiere ProやCapCutで編集・つなぎ合わせてBGMとテロップを後付けする——これが実用的なワークフローです。

関連 → [AIでオリジナルBGMを作る方法](/ai-bgm-creation/)｜[AI音声ナレーションの作り方](/ai-voice-narration/)
...
| 向いている人 | 向いていない人 |
|---|---|
| AI動画生成でイメージ通りの映像を作りたい人 | すでに動画制作のプロとして活動している人 |
| カメラワーク用語を覚えてプロンプト力を上げたい人 | 実写撮影で十分な人 |
| テンプレートをコピペで使いたい初心者 | 10秒以上の長尺動画を1本で完成させたい人 |
